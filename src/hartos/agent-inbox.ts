...
/**
 * Map the classifier's `Domain` to the `ReadModelType` keyspace that AGENT_CONTRACTS
 * is keyed on. ONLY domains that correspond to a real officiated read-model resolve;
 * everything else returns undefined ⇒ "no known agent covers this" (degrade-safe).
 * This is NOT a claim that the request is novel — AGENT_CONTRACTS is incomplete.
 */
function domainToReadModelType(domain: Domain): ReadModelType | undefined {
  switch (domain) {
    ...
    default:
      ...
      // officiated read-model agent today — treat as not-yet-covered, not novel.
      return undefined;
  }
}
...
export function classifyBuildRequest(
  ...
  // ── 2. ALREADY_SOLVED — an existing officiated agent already covers this domain. ──
  const rmType = domainToReadModelType(classification.domain);
  if (rmType) {
    const match = findAgentContract(rmType, contracts);
    if (match) {
      return {
