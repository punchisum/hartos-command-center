 ...
 *   3. Run bridgeCouncilToFactory; if ok → upsert the factory proposal.
 *   4. Collect and return result lines.
 *
...
export async function runCouncilBuildBridgeOnce(env: Env, now: string): Promise<string[]> {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      // 6. Run the bridge.
      let result: ReturnType<typeof bridgeCouncilToFactory>;
      try {
        result = bridgeCouncilToFactory(councilPayload, councilProposalId, now);
      } catch (e) {
